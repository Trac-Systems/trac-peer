...
class PokemonContract extends BaseContract {
  ...
  async catch() {
    const { pokemons } = await this.#ensurePokedex()
    const parsed = JSON.parse(pokemons)

    const pick = GEN1_POKEMON[Math.floor(Math.random() * GEN1_POKEMON.length)];

    const exists = parsed.some(p => p.id === pick.id);
    if (!exists) parsed.push({ id: pick.id, name: pick.name, tx: this.tx });

    await this.#setPokedex(parsed)
  }

  async #ensurePokedex() {
    const pokedex = await this.get(this.#pokedexKey())
    if (pokedex === null) {
      await this.#setPokedex([])
    }

    return await this.get(this.#pokedexKey())
  }

  #pokedexKey() {
    return `app/pokedex/${this.address}`
  }

  async #setPokedex(pokemons) {
    await this.put(this.#pokedexKey(), { pokemons: JSON.stringify(pokemons) });
  }
}
